...
def setup_mms(mparams):
    ...
    u1 = as_vector((phi.dx(1), -phi.dx(0), Constant(0)))  # To be divergence free

    # Stokes stress
    stokes_stress = lambda u, p: -p * Identity(3) + mu*grad(u)
...
def inexact_riesz_map_preconditioner_wRA(AA, W, bcs, mparams, mms):
    ...
    parameters = {
        ...
        "pc_hypre_boomeramg_strong_threshold": .1,# Threshold for being strongly connected
    }
        
    BB_blocks = [petsc.AMG(B0, parameters=parameters),
                 petsc.AMG(B1),
...
if __name__ == '__main__':
    ...
    result_path = f'ds3D_flat_K{args.K_value}_mu{args.mu_value}_alpha{args.alpha_value}_precond{args.preconditioner}_family{args.elm_family}.txt'
